...
k2.metric("😊 Positive", f"{pos_count}", delta=f"{pos_count*100//total_reviews}%")
k3.metric("😡 Negative", f"{neg_count}", delta=f"-{neg_count*100//total_reviews}%", delta_color="inverse")
k4.metric("😐 Neutral", f"{neu_count}", delta=f"{neu_count*100//total_reviews}%", delta_color="off")
...
if show_samples:
 ...
 with tab1:
  pos_samples = df[df['Sentiment'] == 'Positive'][[review_col, 'Confidence %']].head(5)
  st.dataframe(pos_samples, use_container_width=True) if len(pos_samples) > 0 else st.info("No positive reviews found.")

 with tab2:
  neg_samples = df[df['Sentiment'] == 'Negative'][[review_col, 'Confidence %']].head(5)
  st.dataframe(neg_samples, use_container_width=True) if len(neg_samples) > 0 else st.info("No negative reviews found.")

 with tab3:
  neu_samples = df[df['Sentiment'] == 'Neutral'][[review_col, 'Confidence %']].head(5)
  st.dataframe(neu_samples, use_container_width=True) if len(neu_samples) > 0 else st.info("No neutral reviews found.")
